...
impl RedisQueueManager {
    ...
    pub async fn push_transaction(&self, tx: Transaction, priority: u8) -> Result<String, QueueError> {
    ...
    pub async fn mark_complete(&self, tx_id: &str, success: bool) -> Result<Option<Transaction>, QueueError> {
...
impl NonceManager {
    ...
    pub async fn reserve_nonce(&mut self, tx: &Transaction) -> Result<(), QueueError> {
        let nonces = self.address_nonces.entry(tx.from.clone()).or_insert_with(Vec::new);
...
impl TransactionPrioritization {
    ...
    pub fn get_priority(&self, tx: &Transaction) -> u8 {
