...
/*
    common/
    ├── Cargo.toml
    ├── manifest.rs                 -> Tài liệu tham chiếu module path (file này)
    ├── lib.rs                      -> Khai báo module cấp cao, re-export
    ├── src/
    │   ├── api/                    -> Quản lý API gateway và router
    │   │   ├── mod.rs
    │   │   ├── gateway.rs          -> API gateway xử lý và điều phối yêu cầu
    │   │   ├── router.rs           -> Router template và định tuyến yêu cầu
    │   │   ├── middleware.rs       -> Middleware (logging, rate limiting)
    │   │   ├── error_handling.rs   -> Xử lý lỗi và response format
    │   │   ├── docs.rs             -> Documentation cho API
    │   │   ├── gateway.rs          -> API gateway xử lý và điều phối yêu cầu
    │   │   ├── router.rs           -> Router template và định tuyến yêu cầu
    │   │   ├── middleware.rs       -> Middleware (logging, rate limiting)
    │   │   ├── error_handling.rs   -> Xử lý lỗi và response format
