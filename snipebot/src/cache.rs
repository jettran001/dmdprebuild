...
pub struct CacheStats {
    ...
    pub item_count: std::sync::atomic::AtomicUsize,
    ...
    pub hit_ratio: std::sync::atomic::AtomicU64,
}
...
impl CacheStats {
    ...
    pub fn new() -> Self {
        Self {
            item_count: std::sync::atomic::AtomicUsize::new(0),
            hits: std::sync::atomic::AtomicUsize::new(0),
            misses: std::sync::atomic::AtomicUsize::new(0),
            hit_ratio: std::sync::atomic::AtomicU64::new(0),
        }
    }
    ...
    pub fn set_item_count(&self, count: usize) {
        self.item_count.store(count, std::sync::atomic::Ordering::Relaxed);
    }
    ...
    fn update_hit_ratio(&self) {
        ...
        if total > 0 {
            let ratio = (hits as f64 / total as f64 * 100.0) as u64;
            self.hit_ratio.store(ratio, std::sync::atomic::Ordering::Relaxed);
        }
    }
}
...
impl CacheProvider for InMemoryCache {
    ...
    async fn set(&self, key: &str, value: Vec<u8>, ttl_seconds: u64) -> Result<()> {
        ...
        // Update cache
        {
            ...
            // Update stats
            let mut stats = self.stats.write().await;
            stats.set_item_count(cache.len());
        }
...
impl CacheProvider for RedisCache {
    ...
    async fn set(&self, key: &str, value: Vec<u8>, ttl_seconds: u64) -> Result<()> {
        ...
        // Update stats
        let mut stats = self.stats.write().await;
        stats.set_item_count(stats.item_count.load(std::sync::atomic::Ordering::Relaxed) + 1);
    ...
    async fn delete(&self, key: &str) -> Result<()> {
        ...
        let mut stats = self.stats.write().await;
        if stats.item_count.load(std::sync::atomic::Ordering::Relaxed) > 0 {
            stats.item_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
...
mod tests {
    ...
    async fn test_in_memory_cache() {
        ...
        // Test set/get
        if let Err(e) = cache.set("test_key", vec![1, 2, 3], 60).await {
            panic!("Failed to set cache key: {}", e);
        }
        
        let result = match cache.get("test_key").await {
            Ok(data) => data,
            Err(e) => panic!("Failed to get cache key: {}", e),
        };
        assert_eq!(result, Some(vec![1, 2, 3]));
        
        // Test exists
        let exists = match cache.exists("test_key").await {
            Ok(exists) => exists,
            Err(e) => panic!("Failed to check if key exists: {}", e),
        };
        assert!(exists);
        
        let not_exists = match cache.exists("non_existent").await {
            Ok(exists) => exists,
            Err(e) => panic!("Failed to check if key exists: {}", e),
        };
        assert!(!not_exists);
        ...
        // Test delete
        if let Err(e) = cache.delete("test_key").await {
            panic!("Failed to delete cache key: {}", e);
        }
        
        let exists_after_delete = match cache.exists("test_key").await {
            Ok(exists) => exists,
            Err(e) => panic!("Failed to check if key exists after delete: {}", e),
        };
        assert!(!exists_after_delete);
        
        // Test token price
        if let Err(e) = cache.set_token_price(1, "0x123", 100.5, 60).await {
            panic!("Failed to set token price: {}", e);
        }
        
        let price = match cache.get_token_price(1, "0x123").await {
            Ok(price) => price,
            Err(e) => panic!("Failed to get token price: {}", e),
        };
