    ...
    pub fn new(token: String, chat_id: String) -> Result<Self> {
        let client = Client::builder()
            ...
            .map_err(|e| {
                ...
                anyhow!("Không thể tạo HTTP client cho Telegram: {}", e)
            })
            .or_else(|e| {
    ...
    fn format_message(&self, notification_type: &NotificationType, data: &JsonValue) -> String {
        ...
        if let Some(timestamp) = data.get("timestamp").and_then(|v| v.as_i64()) {
            let datetime = chrono::DateTime::from_timestamp(timestamp, 0)
                .map_or_else(
                    || {
                        error!("Invalid timestamp {}, falling back to current time", timestamp);
                        Utc::now()
                    },
                    |dt| dt
                );
...
impl NotificationManager {
    ...
    pub fn new() -> Self {
        Self {
            ...
            rate_limit: RwLock::new(HashMap::<String, usize>::new()),
        }
    }
