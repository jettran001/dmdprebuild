...
pub struct RateLimiterImplStruct {
    /// Tên rate limiter
    name: String,
...
#[async_trait]
impl RateLimiter for RateLimiterImplStruct {
    /// Lấy một RateLimiterImpl cụ thể từ trait chung
    /// 
    /// Đây là phương thức không async để đảm bảo trait tương thích với dyn
    fn as_rate_limiter_impl(&self) -> &dyn RateLimiterImpl {
...
#[async_trait]
impl RateLimiterImpl for RateLimiterImplStruct {
    /// Acquire a permit to make a request
    async fn acquire(&self, priority: Option<RequestPriority>) -> Result<()> {
        // Xử lý theo độ ưu tiên nếu được bật
        if self.config.priority_levels {
            let priority_value = priority.unwrap_or(RequestPriority::Medium);
            let semaphore = match priority_value {
                RequestPriority::Critical | RequestPriority::High => 
                    self.high_priority_semaphore.as_ref().ok_or_else(|| anyhow!("High priority semaphore not initialized for {}", self.name))?,
                RequestPriority::Medium => 
                    self.medium_priority_semaphore.as_ref().ok_or_else(|| anyhow!("Medium priority semaphore not initialized for {}", self.name))?,
                RequestPriority::Low => 
                    self.low_priority_semaphore.as_ref().ok_or_else(|| anyhow!("Low priority semaphore not initialized for {}", self.name))?,
            };
    ...
    async fn try_acquire(&self, priority: Option<RequestPriority>) -> bool {
        ...
        if self.config.priority_levels {
            let priority_value = priority.unwrap_or(RequestPriority::Medium);
            let semaphore_result = match priority_value {
                RequestPriority::Critical | RequestPriority::High => 
                    self.high_priority_semaphore.as_ref(),
                RequestPriority::Medium => 
                    self.medium_priority_semaphore.as_ref(),
                RequestPriority::Low => 
                    self.low_priority_semaphore.as_ref(),
            };
            
            // Nếu không có semaphore hoặc không thể lấy permit, trả về false
            if let Some(semaphore) = semaphore_result {
                if semaphore.try_acquire().is_err() {
                    return false;
                }
            } else {
                // Log lỗi và trả về false nếu không tìm thấy semaphore
                warn!("Semaphore not initialized for priority {:?} in {}", priority_value, self.name);
                return false;
            }
        }
...
where
    ...
    T: Send,
{
    // Get the rate limiter implementation
    let limiter_impl = limiter.as_rate_limiter_impl();
    
    // Acquire a permit
    match limiter_impl.acquire(priority).await {
        Ok(()) => {
...
impl ApiRateLimiterManager {
    ...
    pub async fn register_limiter(&self, name: &str, config: Option<RateLimitConfig>) -> Result<Arc<dyn RateLimiter>, anyhow::Error> {
        ...
        let actual_config = match config {
            Some(cfg) => cfg,
            None => self.default_configs.get(name)
                .cloned()
                .unwrap_or_default()
        };
        
        // Tạo limiter mới
        let limiter = Arc::new(RateLimiterImplStruct::new(name, actual_config));
    ...
    pub async fn get_or_create_limiter(&self, name: &str, config: Option<RateLimitConfig>) -> Result<Arc<dyn RateLimiter>, anyhow::Error> {
        ...
        match self.register_limiter(name, config).await {
            ...
            Err(e) => {
                ...
                let temp_limiter = Arc::new(RateLimiterImplStruct::new(name, config));
                Ok(temp_limiter)
            }
        }
    }
        ...
        T: Send,
    {
        ...
        let limiter = match self.get_or_create_limiter(api_name, config.clone()).await {
            Ok(limiter) => limiter,
            Err(e) => {
...
mod tests {
    ...
    async fn test_rate_limiter_impl() {
        ...
        let limiter = RateLimiterImplStruct::new("test", config);
        ...
        match limiter.acquire(None).await {
            Ok(_) => (),
            Err(e) => panic!("Failed to acquire in test: {}", e),
        };
    }
    ...
    async fn test_api_rate_limiter_manager() {
        ...
        for _ in 0..10 {
            ...
            match result {
                ...
                Err(e) => panic!("API call failed in batch: {}", e),
            };
        }
