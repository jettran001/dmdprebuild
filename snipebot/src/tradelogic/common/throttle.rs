...
/// Cấu trúc rate limit cho các hành động
#[derive(Debug, Clone)]
pub struct RateLimit {
    /// Thời điểm hành động cuối cùng
    pub last_action: Instant,
    
    /// Khoảng thời gian tối thiểu giữa các hành động (ms)
...
/// Kiểm tra và thực hiện throttle nếu cần
pub async fn check_throttle(&self, action_type: &str) -> bool {
    // Tạo một key cho action này nếu chưa có
    let action_key = action_type.to_string();
    
    // Lấy rate limit hiện tại hoặc tạo mới
    let mut rate_limits = self.rate_limits.lock().await;
    let rate_limit = rate_limits.entry(action_key.clone()).or_insert_with(|| {
        // Tạo rate limit mặc định dựa trên loại hành động
        let interval_ms = match action_type {
            "trade" => 1000,   // 1 giây giữa các giao dịch
            "scan" => 5000,    // 5 giây giữa các lần quét
            "query" => 200,    // 200ms giữa các truy vấn
            _ => 500,          // 500ms mặc định cho các hành động khác
        };
        
        RateLimit {
            last_action: Instant::now() - Duration::from_secs(3600), // Đặt thời điểm cuối là 1 giờ trước để cho phép hành động đầu tiên
    ...
    let now = Instant::now();
    let elapsed = now.duration_since(rate_limit.last_action);
    let min_interval = Duration::from_millis(rate_limit.min_interval_ms);
    
    if elapsed < min_interval {
        // Chưa đủ thời gian, hãy chờ
        let wait_time = min_interval - elapsed;
        debug!("Throttling {}: waiting {:?}", action_type, wait_time);
        
        // Cập nhật thời gian cuối nhưng không chờ
        rate_limit.last_action = now + wait_time;
        return true; // Đã throttle
    }
    
    // Đã đủ thời gian, cập nhật thời gian cuối và cho phép tiếp tục
    rate_limit.last_action = now;
    false // Không throttle
}
