...
/// Determines the type of trader based on transaction history
///
/// # Arguments
/// * `transactions` - Transaction history
///
/// # Returns
/// * `TraderBehaviorType` - Type of trader
pub fn determine_trader_type(transactions: &[MempoolTransaction]) -> TraderBehaviorType {
...
/// Determines the expertise level of a trader based on transaction history
///
/// # Arguments
/// * `transactions` - Transaction history
///
/// # Returns
/// * `TraderExpertiseLevel` - Expertise level of trader
pub fn determine_expertise_level(transactions: &[MempoolTransaction]) -> TraderExpertiseLevel {
    if transactions.is_empty() {
...
/// Analyzes gas usage behavior of a trader
///
/// # Arguments
/// * `transactions` - Transaction history
///
/// # Returns
/// * `GasBehavior` - Gas usage behavior
pub fn analyze_gas_behavior(transactions: &[MempoolTransaction]) -> GasBehavior {
    if transactions.is_empty() {
        return GasBehavior {
...
pub fn convert_token_safety_to_security_check(
    ...
    // Convert TokenIssue to TokenIssueDetail
    let issue_details = issues.iter().map(|issue| {
        TokenIssueDetail {
            issue_type: match issue {
                TokenIssue::Honeypot => TokenIssueType::Honeypot,
                TokenIssue::HighTax | TokenIssue::HighBuyTax | TokenIssue::HighSellTax => TokenIssueType::FeeManipulation,
                TokenIssue::BlacklistFunction | TokenIssue::HasBlacklist | TokenIssue::HasWhitelist => TokenIssueType::Blacklist,
                TokenIssue::UnlimitedMintAuthority => TokenIssueType::MintFunction,
                TokenIssue::ExternalCalls => TokenIssueType::MaliciousTransfer,
                TokenIssue::UnverifiedContract => TokenIssueType::UnverifiedCode,
                TokenIssue::ProxyContract | TokenIssue::ProxyImplementation => TokenIssueType::Proxy,
                _ => TokenIssueType::Other,
            },
            description: format!("{:?}", issue),
            severity: 80, // Default high severity
        }
    }).collect();
    ...
    SecurityCheckResult {
        ...
        is_honeypot: token_safety.is_honeypot,
        is_contract_verified: token_safety.is_contract_verified,
        rug_pull_risk: false, // Default value, update if needed
...
pub async fn analyze_token_security(
    ...
    let contract_issues = detect_token_issues(&contract_info);
    ...
    // Merge issues from contract_issues (avoiding duplicates)
    for issue in contract_issues {
        ...
        if !result.issues.iter().any(|detail| detail.issue_type == issue_type) {
            result.issues.push(TokenIssueDetail {
                issue_type,
                description: format!("{:?}", issue),
                severity: 80, // Default high severity
            });
    ...
    if let Some(created_at) = token_info.created_at {
        ...
        if age_in_days < 1 {
            // Use LowLiquidity as a proxy for NewToken since it represents similar risk
            result.issues.push(TokenIssueDetail {
                issue_type: TokenIssueType::Other,
                description: "Token created less than 1 day ago".to_string(),
                severity: 80,
            });
            result.risk_score += 30;
        } else if age_in_days < 7 {
            // Use Other as a proxy for RecentToken
            result.issues.push(TokenIssueDetail {
                issue_type: TokenIssueType::Other,
...
///
/// # Returns
/// * `bool` - True if front-running pattern detected
...
///
/// # Arguments
/// * `events` - List of liquidity events
///
/// # Returns
/// * `bool` - True if abnormal liquidity pattern detected
...
/// Detect arbitrage opportunities from transactions and DEX prices
///
/// # Arguments
/// * `transactions` - List of transactions
/// * `current_dex_prices` - Current prices on different DEXes
///
/// # Returns
/// * Vector of (token1, token2, dex, profit_percentage) tuples
pub fn detect_arbitrage_opportunities(
    ...
    // Extract token pairs from transactions
    let mut token_pairs = HashSet::new();
    for tx in transactions {
        if let Some(swap_info) = &tx.swap_info {
            token_pairs.insert((
                swap_info.from_token.address.clone(),
                swap_info.to_token.address.clone()
            ));
        }
    }
    ...
    for (token1, token2) in token_pairs {
        ...
        for (dex, prices) in current_dex_prices {
            if let Some(price) = prices.get(&format!("{}-{}", token1, token2)) {
                if *price > best_price {
                    best_price = *price;
                    best_dex = dex.clone();
                }
        ...
        // Find worst price for reverse calculation
        let mut worst_price = f64::MAX;
        for (dex, prices) in current_dex_prices {
            if let Some(price) = prices.get(&format!("{}-{}", token2, token1)) {
                if *price < worst_price && *price > 0.0 {
                    worst_price = *price;
                }
            }
        }
        ...
        if best_price > 0.0 && worst_price < f64::MAX {
            let profit_percentage = (1.0 / worst_price * best_price - 1.0) * 100.0;
            
            // Only include if profit is significant
            if profit_percentage > 1.0 {
                opportunities.push((token1, token2, best_dex, profit_percentage));
            }
        }
    }
...
pub fn find_transactions_for_token(
    ...
    transactions.iter()
        .filter(|tx| {
            if let Some(swap_info) = &tx.swap_info {
                swap_info.from_token.address == token_address || 
                swap_info.to_token.address == token_address
            } else if let Some(liq_info) = &tx.liquidity_info {
                liq_info.token_address == token_address
            } else {
                false
            }
        })
...
/// Detect new tokens from transactions
///
/// # Arguments
/// * `transactions` - List of transactions
/// * `known_tokens` - Set of already known tokens
///
/// # Returns
/// * Vector of (token_address, creator_address, initial_liquidity, timestamp)
    ...
    for tx in transactions {
        ...
        if tx.transaction_type == TransactionType::TokenCreation {
            if let Some(token_info) = &tx.token_info {
                if !known_tokens.contains(&token_info.address) {
                    new_tokens.push((
                        token_info.address.clone(),
                        tx.from_address.clone(),
                        0.0, // Initial liquidity unknown at creation
                        tx.timestamp
                    ));
                }
            }
        ...
        // Check for initial liquidity additions
        if tx.transaction_type == TransactionType::AddLiquidity {
            if let Some(liq_info) = &tx.liquidity_info {
                if !known_tokens.contains(&liq_info.token_address) {
                    // Check if this is the first liquidity event for this token
                    let is_first = transactions.iter()
                        .filter(|other_tx| {
                            other_tx.transaction_type == TransactionType::AddLiquidity &&
                            other_tx.liquidity_info.as_ref().map_or(false, |info| 
                                info.token_address == liq_info.token_address
                            )
                        })
                        .all(|other_tx| other_tx.timestamp >= tx.timestamp);
                    
                    if is_first {
                        new_tokens.push((
                            liq_info.token_address.clone(),
                            tx.from_address.clone(),
                            liq_info.value,
                            tx.timestamp
                        ));
