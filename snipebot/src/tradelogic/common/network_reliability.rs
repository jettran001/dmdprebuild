...
use crate::tradelogic::common::types::NetworkStatus;
...
impl NetworkReliabilityManager {
    ...
    /// 
    /// Hàm này thực hiện một request với cơ chế retry tự động và circuit breaker.
    /// 
    /// # Arguments
    /// * `self` - NetworkReliabilityManager instance
    /// * `service_id` - ID của service
    /// * `retry_strategy` - Chiến lược retry
    /// * `future_fn` - Future function cần thực thi
    /// 
    /// # Returns
    /// * `Result<T>` - Kết quả của request
    /// 
    /// # Errors
    /// * Trả về lỗi nếu circuit breaker đang mở
    /// * Trả về lỗi nếu request thất bại sau tất cả các lần retry
        ...
        Fut: std::future::Future<Output = Result<T>>,
    {
        ...
        for attempt in 0..=retry_strategy.max_retries {
            ...
            if !can_proceed {
                ...
                return Err(anyhow!(
                    "Circuit breaker open for service {}, network status: {:?}", 
                    service_id, 
                    status
                ));
            }
                ...
                Err(e) => {
                    ...
                    if attempt == retry_strategy.max_retries {
                        return Err(e.context(format!(
                            "Failed after {} retries for service {}", 
                            attempt, 
                            service_id
                        )));
                    }
                    ...
                    warn!(
                        "Request to {} failed (attempt {}/{}), retrying after {}ms: {:?}",
                        service_id, 
                        attempt + 1, 
        ...
        Err(anyhow!(
            "Unexpected error: retry loop exited without result for service {}", 
            service_id
        ))
    }
        ...
        F: Fn(&str) -> Pin<Box<dyn Future<Output = ()> + Send>> + Send + Sync + 'static,
    {
        ...
        let service_callbacks = callbacks.entry(service_id).or_insert_with(Vec::new);
        service_callbacks.push(callback_arc);
    }
    ...
    /// Đăng ký health check function cho một service
    pub async fn register_health_check<F, Fut>(&self, service_id: &str, health_check_fn: F) -> Result<()>
    where
        F: Fn() -> Fut + Send + Sync + Clone + 'static,
        Fut: std::future::Future<Output = bool> + Send + 'static,
    {
        // Tạo wrapper để chuyển đổi từ F và Fut sang trait object
        ...
        // Thêm vào danh sách các service được giám sát
        {
            let mut status = self.network_status.write().await;
            if !status.contains_key(&service_id) {
                status.insert(service_id.clone(), NetworkStatus::Healthy);
            }
        }
        
        // Tạo circuit breaker nếu chưa có
        self.get_or_create_breaker(&service_id).await;
        
        // Đăng ký function để kiểm tra sức khỏe
        // TODO: Implement health check scheduler
        
        Ok(())
    }

    /// Lên lịch thử kết nối lại với một service
        ...
        Fut: std::future::Future<Output = bool> + Send + 'static,
    {
        ...
        // Lấy hoặc tạo interval
        let interval_ms = {
            let intervals = self.reconnect_interval_ms.read().await;
            *intervals.get(&service_id).unwrap_or(&5000)
        };
        
        // Đặt trạng thái network là Reconnecting
        {
            let mut status = self.network_status.write().await;
            status.insert(service_id.clone(), NetworkStatus::Reconnecting);
        }
        
        // Cập nhật lần thử kết nối cuối
        {
            let mut last_attempts = self.last_reconnect_attempt.write().await;
            last_attempts.insert(service_id.clone(), Instant::now());
        }
        
        // Tạo một boxed closure để thử kết nối lại
        let reconnect_fn = Arc::new(move || -> Pin<Box<dyn Future<Output = bool> + Send>> {
            let fut = health_check_fn();
            Box::pin(async move { fut.await })
        });
        
        // Tạo task mới để thử kết nối lại
        let self_clone = self.clone();
        let service_id_clone = service_id.clone();
        let reconnect_fn_clone = reconnect_fn.clone();
        ...
        tokio::spawn(async move {
            ...
            loop {
                // Kiểm tra xem đã vượt quá số lần thử không
                if attempts >= max_attempts {
                    error!("Failed to reconnect to {} after {} attempts", service_id_clone, attempts);
                    
                    // Đặt trạng thái network là Failed
                    let mut status = self_clone.network_status.write().await;
                    status.insert(service_id_clone, NetworkStatus::Failed);
                ...
                info!("Attempting to reconnect to {} (attempt {}/{})", service_id_clone, attempts, max_attempts);
                
                // Thử kết nối lại
                let fut = reconnect_fn_clone();
                let result = fut.await;
                
                if result {
                    info!("Successfully reconnected to {} after {} attempts", service_id_clone, attempts);
                    
                    // Đặt trạng thái network là Healthy
                    let mut status = self_clone.network_status.write().await;
                    status.insert(service_id_clone.clone(), NetworkStatus::Healthy);
                    
                    // Reset circuit breaker
                    self_clone.reset(&service_id_clone).await;
                    
                    // Gọi các callback khi kết nối lại thành công
                    let callbacks = {
                        let callbacks = self_clone.reconnect_callbacks.read().await;
                        if let Some(cbs) = callbacks.get(&service_id_clone) {
                            cbs.clone()
                        } else {
                            Vec::new()
                        }
                    ...
                    for callback in callbacks {
                        let fut = callback(&service_id_clone);
                        tokio::spawn(fut);
                    }
                    
                    break;
                } else {
                    warn!("Failed to reconnect to {} (attempt {}/{})", service_id_clone, attempts, max_attempts);
                }
            }
            
            // Cập nhật số lần thử kết nối lại
            let mut reconnect_attempts = self_clone.reconnect_attempts.write().await;
            reconnect_attempts.insert(service_id_clone, attempts);
        });
    }
