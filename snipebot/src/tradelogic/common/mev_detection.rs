...
pub async fn analyze_mempool_for_mev(
    ...
    for tx in &related_txs {
        ...
        if let Ok(pattern) = detect_front_running(tx, mempool_txs, 5000).await {
        ...
        if let Ok(pattern) = detect_sandwich_attack(tx, mempool_txs, 5000).await {
        ...
        if let Ok(pattern) = detect_high_frequency_trading(tx, mempool_txs, 30).await {
            if let Some(SuspiciousPattern::HighFrequencyTrading) = pattern {
