...
/// Resource manager for handling rate limits and resource allocation
...
impl ResourceManager {
    ...
    /// Execute a function with rate limiting for a specific resource
        ...
        Fut: std::future::Future<Output = Result<T>>,
    {
        ...
        let limiter = self.get_or_create_rate_limiter(resource).await;
        
        // Wait for rate limit
        limiter.wait_for_permit(priority).await?;
        
        // Execute future
        future_fn().await
    }
    
    /// Execute a function with blockchain rate limiting
    ...
    /// Safely convert from usize to u64
    #[inline]
    fn usize_to_u64(val: usize) -> u64 {
        // usize always converts to u64 on 64-bit architecture
        // But needs checking on 32-bit
    ...
    /// Safely convert from u64 to usize
    #[inline]
    fn u64_to_usize(val: u64) -> Option<usize> {
        #[cfg(target_pointer_width = "32")]
        {
