...
/// - Estimating costs and times for cross-chain operations
/// - Supporting multiple bridge protocols (LayerZero, Wormhole, etc.)
#[async_trait]
...
///
/// TradeCoordinator chịu trách nhiệm:
/// - Điều phối cơ hội giao dịch giữa các executor khác nhau (MEV, Smart Trade, Manual)
/// - Quản lý đặt trước cơ hội để tránh xung đột
/// - Thu thập số liệu thống kê về cơ hội được chia sẻ
/// - Duy trì các tham số toàn cục được chia sẻ giữa các executor
/// - Đảm bảo thread-safety khi nhiều executor truy cập state đồng thời
