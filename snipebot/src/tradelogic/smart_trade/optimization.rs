...
impl SmartTradeExecutor {
    ...
    /// Execute a batch trade of multiple tokens in a single transaction
    ///
    /// This method allows executing multiple token purchases in a single transaction,
    /// which can save gas and improve efficiency.
    ///
    /// # Arguments
    /// * `chain_id` - Chain ID for the transaction
    /// * `token_addresses` - List of token addresses to buy
    /// * `amounts` - List of amounts to buy for each token
    /// * `adapter` - EVM adapter for the chain
    ///
    /// # Returns
    /// * `anyhow::Result<String>` - Transaction hash if successful, error if failed
    pub async fn batch_trade(&self, chain_id: u32, token_addresses: &[String], amounts: &[f64], adapter: &Arc<EvmAdapter>) -> anyhow::Result<String> {
        if token_addresses.len() != amounts.len() {
            return Err(anyhow::anyhow!("Number of tokens and amounts do not match"));
        }
        
        if token_addresses.is_empty() {
            return Err(anyhow::anyhow!("Token list is empty"));
        }
        
        info!("Starting batch trade with {} tokens", token_addresses.len());
        
        let mut trade_params_list = Vec::new();
        
        // Prepare trade parameters
        for (i, token_address) in token_addresses.iter().enumerate() {
            // Find optimal pool for each token
            let router_address = if let Some(optimal_pool) = self.select_optimal_pool(chain_id, token_address, adapter).await {
                optimal_pool
            } else {
                // Use default router if no optimal pool found
                return Err(anyhow::anyhow!("Could not find optimal pool for token {}", token_address));
            };
            ...
            let trade_params = crate::types::TradeParams {
                ...
                amount: amounts[i],
                slippage: 2.0, // Default 2% slippage
        ...
        match adapter.execute_batch_transaction(&trade_params_list).await {
            Ok(tx_hash) => {
                info!("Batch trade executed successfully with hash: {}", tx_hash);
                Ok(tx_hash)
            },
            Err(e) => {
                error!("Batch trade failed: {}", e);
                Err(anyhow::anyhow!("Batch trade execution failed: {}", e))
            }
        }
    }
    
    /// Apply anti-MEV protection to a transaction
    ///
    /// This method adds protection against MEV attacks like frontrunning and sandwich attacks
    /// by using techniques such as private transactions, timing randomization, etc.
    ///
    /// # Arguments
    /// * `chain_id` - Chain ID for the transaction
    ...
    ///
    /// # Returns
    /// * `anyhow::Result<(String, String)>` - (Transaction hash, Protection parameters) if successful
    pub async fn anti_mev_protection(&self, chain_id: u32, token_address: &str, amount: f64, adapter: &Arc<EvmAdapter>) -> anyhow::Result<(String, String)> {
        info!("Applying anti-MEV protection for token {} on chain {}", token_address, chain_id);
        
        // Check if private transactions are supported on this chain
        let supports_private_tx = self.check_private_tx_support(chain_id).await;
        
        // Determine protection strategy
        let protection_type = if supports_private_tx {
            "private_transaction"
        } else {
        ...
        let trade_params = crate::types::TradeParams {
            ...
            amount,
            slippage: 1.0, // Lower slippage for MEV protection
            trade_type: crate::types::TradeType::Buy,
            deadline_minutes: 2, // Shorter deadline
            router_address,
        };
        ...
        match protection_type {
            "private_transaction" => {
                // Submit as private transaction
                match adapter.submit_private_transaction(&trade_params).await {
                    Ok(tx_hash) => {
                        info!("Private transaction submitted with hash: {}", tx_hash);
                        Ok((tx_hash, "private_transaction".to_string()))
                    },
                    Err(e) => {
                        error!("Private transaction failed: {}", e);
                        Err(anyhow::anyhow!("Private transaction failed: {}", e))
                    }
                }
            },
            "timing_randomization" => {
                ...
                // Execute with randomized gas settings
                match adapter.execute_transaction_with_custom_gas(&trade_params, None, None).await {
                    Ok(tx_hash) => {
                        info!("Transaction with timing randomization submitted: {}", tx_hash);
                        Ok((tx_hash, format!("timing_randomization:delay={}ms", delay)))
                    },
                    Err(e) => {
                        error!("Transaction with timing randomization failed: {}", e);
                        Err(anyhow::anyhow!("Transaction failed: {}", e))
                    }
                }
            },
            _ => Err(anyhow::anyhow!("Unknown protection type: {}", protection_type)),
        }
    }
}
