...
use crate::analys::token_status::types::{TokenIssueType};
...
pub async fn analyze_token_contract(token_address: &str, adapter: &Arc<EvmAdapter>) -> Result<TokenSafety> {
    ...
    let has_blacklist = contract_info.has_blacklist.unwrap_or(false); // Lấy từ contract_info thay vì token_safety
