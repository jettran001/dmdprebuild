...
use crate::tradelogic::traits::{SharedOpportunity, SharedOpportunityType};
use crate::tradelogic::common::{
    NetworkStatus, RetryStrategy, RetryConfig, with_retry
};
...
pub async fn execute_buy(
    ...
    // Lấy slippage từ params hoặc mặc định từ config
    let slippage = if params.slippage > 0.0 {
        params.slippage
    } else {
    ...
    let result = match retry_async(
        ...
        Ok(tx) => {
            ...
            if let Some(take_profit) = params.take_profit {
                trade_tracker.target_price = Some(current_price * (1.0 + take_profit / 100.0));
            }
...
pub async fn execute_opportunity(
    ...
    // Lấy config để truy cập wallet address mặc định
    let config = executor.config.read().await;
    let default_wallet = config.trading.smart_trade.default_wallet_address.clone()
        .unwrap_or_else(|| "0x0".to_string());
    
    // Lấy thông tin token từ opportunity
    let token_address = opportunity.tokens.get(0)
        .ok_or_else(|| anyhow::anyhow!("No token address in opportunity"))?
        .clone();
    
    // Tạo token pair từ thông tin hiện có
    ...
    // Xác định loại giao dịch dựa vào opportunity type
    let trade_type = match &opportunity.opportunity_type {
        SharedOpportunityType::NewToken => TradeType::Buy,
        SharedOpportunityType::PriceMovement => TradeType::Buy,
        _ => TradeType::Buy, // Mặc định là mua
    };
    
    // Tạo trade params từ opportunity
    let params = TradeParams {
        chain_type: ChainType::EVM(opportunity.chain_id),
        token_address: token_address.clone(),
        token_pair: token_pair,
        trade_type: trade_type,
        amount: opportunity.estimated_profit_usd, // Sử dụng estimated_profit_usd làm amount
        slippage: config.default_slippage, // Sử dụng mặc định từ config
        deadline_minutes: 30, // Sử dụng mặc định
        router_address: "".to_string(),
        wallet_address: default_wallet, // Sử dụng wallet mặc định từ config
        gas_limit: None,
        gas_price: None,
        strategy: None,
        stop_loss: None,
...
async fn save_trade_state(executor: &SmartTradeExecutor, tracker: &TradeTracker) -> Result<()> {
    ...
    let mut trade_history = executor.trade_history.write().await;
    
    // Lưu hoặc cập nhật giao dịch trong history
    let found = trade_history.iter_mut().find(|tr| tr.trade_id == tracker.trade_id);
    if found.is_none() {
        // Tạo TradeResult từ TradeTracker
...
impl TradeHandler {
    ...
    async fn build_trade_result(
        ...
        let result = TradeResult {
            ...
            params: params.clone(),
            status,
            created_at: now - 3600, // Giả sử giao dịch được tạo 1 giờ trước
            completed_at: if status == TradeStatus::Completed || status == TradeStatus::Failed {
                Some(now)
            } else {
                None
            },
            tx_hash: Some(tx_hash.to_string()),
            actual_amount: None, // Cần lấy từ receipt
            actual_price: None, // Cần lấy từ receipt
            fee: None, // Cần lấy từ receipt
            profit_loss: None,
            error: if status == TradeStatus::Failed {
                Some("Transaction failed".to_string())
            } else {
                None
            },
            explorer_url: Some(get_explorer_url(params.chain_id())),
        };
