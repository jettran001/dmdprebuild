...
impl SmartTradeAnalysisClient {
    ...
    pub fn new(chain_adapters: HashMap<u32, Arc<EvmAdapter>>) -> Self {
        ...
        let providers_result = crate::tradelogic::create_analysis_providers(chain_adapters);
        
        // Xử lý lỗi một cách an toàn
        let (mempool_provider, token_provider, risk_provider, mev_provider) = match providers_result {
            Ok(providers) => providers,
            Err(e) => {
                tracing::error!("Failed to create analysis providers: {}", e);
                panic!("Cannot initialize SmartTradeAnalysisClient: {}", e);
            }
        };
    ...
    /// 
    /// # Returns
    /// * `Result<TokenStatus>` - Trạng thái token bao gồm các thông tin về an toàn, thuế, v.v.
    ///
    /// # Errors
    /// * Trả về lỗi nếu không thể kết nối với provider
    /// * Trả về lỗi nếu không thể phân tích token
    pub async fn get_token_status(&self, chain_id: u32, token_address: &str) -> Result<TokenStatus> {
        // Lấy thông tin phân tích token từ provider
        let safety_status = self.token_provider.analyze_token_safety(chain_id, token_address).await
            .context("Không thể phân tích an toàn của token")?;
        ...
        // Cập nhật các trường từ safety_status
        token_status.honeypot = safety_status.is_honeypot;
        token_status.contract_verified = safety_status.is_contract_verified;
        token_status.blacklist = safety_status.issues.iter().any(|issue| 
            matches!(issue.issue_type, TokenIssueType::Blacklist)
        );
        
        // Lấy thông tin thuế từ provider
        if let Ok(tax_info) = self.token_provider.get_token_tax_info(chain_id, token_address).await {
            token_status.tax_buy = tax_info.buy_tax;
            token_status.tax_sell = tax_info.sell_tax;
        }
        
        // Lấy thông tin thanh khoản từ provider
        if let Ok(liquidity_info) = self.token_provider.get_token_liquidity(chain_id, token_address).await {
            token_status.low_liquidity = liquidity_info.total_liquidity_usd < 10000.0; // Dưới 10k USD là thanh khoản thấp
        }
        
        // Cập nhật các vấn đề từ safety_status
        token_status.issues = safety_status.issues.iter().map(|issue| {
            (match issue.issue_type {
                TokenIssueType::Honeypot => TokenIssue::Honeypot,
                TokenIssueType::FeeManipulation => TokenIssue::HighTax,
                TokenIssueType::Blacklist => TokenIssue::BlacklistFunction,
                TokenIssueType::MintFunction => TokenIssue::UnlimitedMintAuthority,
                TokenIssueType::MaliciousTransfer => TokenIssue::ExternalCalls,
                _ => TokenIssue::Other,
