...
impl GasOptimizer {
    ...
    pub async fn select_optimal_pool(
        ...
        // Lấy danh sách pools cho token
        let pools = self.get_token_pools(adapter.clone(), token_address).await?;
        
        if pools.is_empty() {
            bail!("No liquidity pools found for token: {}", token_address);
        }
        
        // Sắp xếp pools theo thanh khoản để chọn pool tốt nhất
        let mut sorted_pools = pools;
        sorted_pools.sort_by(|a, b| b.liquidity_usd.partial_cmp(&a.liquidity_usd).unwrap_or(std::cmp::Ordering::Equal));
        
        // Chọn pool có thanh khoản cao nhất
        let best_pool = sorted_pools.first().unwrap().clone();
        
        info!("Selected pool for token {}: {} (DEX: {}, Liquidity: ${:.2})",
            token_address, best_pool.pool_address, best_pool.dex_id, best_pool.liquidity_usd);
    ...
    pub async fn get_token_pools(
        ...
        // Lấy thông tin token price
        let token_price = adapter.get_token_price(token_address).await.unwrap_or(0.0);
        
        // Khởi tạo vector chứa thông tin pools
        let mut pools = Vec::new();
        
        // Khởi tạo pool chính từ thông tin liquidity
        let main_pool = PoolInfo {
            pool_address: liquidity_info.lp_token_address.unwrap_or_else(|| "unknown".to_string()),
            dex_id: liquidity_info.dex_name.clone(),
            base_token_address: liquidity_info.base_token_address.clone(),
            base_token_name: liquidity_info.base_token_symbol.unwrap_or_else(|| "ETH".to_string()),
            token_price,
        ...
        // Nếu có thêm liquidity trên các DEX khác
        if let Some(dex_liquidity) = &liquidity_info.dex_liquidity {
            for (dex_name, liq_info) in dex_liquidity {
                // Skip if same as main pool
                if dex_name == &liquidity_info.dex_name {
                    continue;
                }
                
                let pool = PoolInfo {
                    pool_address: liq_info.pool_address.clone().unwrap_or_else(|| "unknown".to_string()),
                    dex_id: dex_name.clone(),
                    base_token_address: liq_info.base_token_address.clone().unwrap_or_else(|| "unknown".to_string()),
                    base_token_name: liq_info.base_token_symbol.clone().unwrap_or_else(|| "ETH".to_string()),
                    token_price: liq_info.token_price.unwrap_or(token_price),
                    liquidity_usd: liq_info.liquidity_usd,
                    fee_percent: liq_info.fee_percent.unwrap_or(0.3),
                    suggested_gas_price: self.suggest_gas_price(adapter.clone(), 5, None).await.unwrap_or(5.0),
                    is_verified: liq_info.verified.unwrap_or(true),
                };
        ...
        debug!("Found {} liquidity pools for token {}", pools.len(), token_address);
    ...
    pub async fn optimize_trade_parameters(
        ...
        // Tính toán slippage dựa trên thanh khoản và kích thước giao dịch
        let amount_usd = amount * optimal_pool.token_price;
        let liquidity_usd = optimal_pool.liquidity_usd;
        
        // Ước tính slippage dựa trên pool
        let estimated_slippage = self.estimate_slippage_for_pool(
            optimal_pool.clone(),
            amount_usd,
            _is_buy
        ).await?;
    ...
    /// Ước tính slippage dựa trên pool và kích thước giao dịch
    pub async fn estimate_slippage_for_pool(
        &self,
        pool: PoolInfo,
        amount_usd: f64,
        is_buy: bool
        ...
        // Kiểm tra đầu vào
        if amount_usd <= 0.0 || pool.liquidity_usd <= 0.0 {
            return Ok(0.5); // Default slippage 0.5%
        }
        
        // Tính tỷ lệ amount/liquidity
        let ratio = amount_usd / pool.liquidity_usd;
        
        // Tính toán thông số k của AMM (constant product)
        // Giả sử x * y = k, với x và y là reserves của hai token
        
        // Giả định tỷ lệ reserves là 50/50
        let reserve_ratio = 0.5;
        
        // Ước tính reserves của base token trong USD
        let base_reserve_usd = pool.liquidity_usd * reserve_ratio;
        
        // Ước tính reserves của token trong USD
        let token_reserve_usd = pool.liquidity_usd * (1.0 - reserve_ratio);
        
        // Tính slippage dựa trên công thức AMM
        let slippage = if is_buy {
            // Công thức slippage khi mua:
            // Δp = (amount_in / (reserve_in + amount_in))
            let amount_ratio = amount_usd / (base_reserve_usd + amount_usd);
            amount_ratio * 100.0 // Chuyển thành phần trăm
        } else {
            // Công thức slippage khi bán:
            // Δp = (amount_out / reserve_out)
            let amount_ratio = amount_usd / token_reserve_usd;
            amount_ratio * 100.0 // Chuyển thành phần trăm
        };
        
        // Điều chỉnh slippage dựa trên fee
        let adjusted_slippage = slippage * (1.0 + pool.fee_percent / 100.0);
        
        // Giới hạn slippage trong khoảng hợp lý
        // Slippage tối thiểu là 0.1% và tối đa là 10%
        let final_slippage = adjusted_slippage.max(0.1).min(10.0);
        
        debug!(
            "Estimated slippage for trade size ${:.2} in pool {} with ${:.2}M liquidity: {:.2}%",
            amount_usd, pool.pool_address, pool.liquidity_usd / 1_000_000.0, final_slippage
        );
