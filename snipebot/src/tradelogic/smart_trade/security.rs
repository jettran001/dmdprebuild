...
impl SmartTradeExecutor {
    ...
    pub async fn analyze_security_risk(
        ...
        let price_history = adapter.get_token_price_history(token_address, one_day_ago, now, 24).await.ok();
    ...
    pub async fn full_safety_analysis(
        ...
        match adapter.get_token_transaction_history(token_address).await {
            Ok(txs) => {
                // Phân tích các giao dịch gần đây
                let mut suspicious_txs = Vec::new();
                for tx in txs {
                    if tx.event_type == "remove" && tx.amount_usd > 5000.0 {
                        suspicious_txs.push(tx);
                    }
                }
                
                if !suspicious_txs.is_empty() {
                    issues.push(format!("Phát hiện {} lần rút thanh khoản lớn trong 30 ngày gần đây", suspicious_txs.len()));
                }
            },
            Err(e) => {
