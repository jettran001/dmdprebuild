...
impl MevStrategy {
    ...
    pub async fn create_new_token_opportunity(&self, chain_id: u32, token_address: String, token_info: NewTokenInfo, token_status: TokenStatus, alert: MempoolAlert) {
        ...
        let potential_trade_value = token_info.initial_liquidity_usd.unwrap_or(1000.0);
        ...
        if risk_score <= config.max_risk_score && estimated_net_profit_usd > config.min_profit_threshold_usd {
            ...
            let base_token = token_info.pair_with.unwrap_or_else(|| {
                if chain_id == 1 { "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2".to_string() } // WETH
                else if chain_id == 56 { "0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c".to_string() } // WBNB
                else { "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2".to_string() } // Default to WETH
            });
            ...
            let opportunity = MevOpportunity {
                ...
                execution_method: MevExecutionMethod::StandardTransaction,
                specific_params: {
                    let mut params = HashMap::new();
                    params.insert("token_address".to_string(), token_address);
                    params.insert("token_safety".to_string(), format!("{:?}", token_status.evaluate_safety()));
                    params.insert("risk_score".to_string(), risk_score.to_string());
                    
                    // Thêm thông tin chi tiết về token
                    ...
                    if let Some(total_supply) = token_info.total_supply {
                        params.insert("total_supply".to_string(), total_supply);
                    }
                    if let Some(creator) = token_info.creator {
                        params.insert("creator".to_string(), creator);
                    }
    ...
    pub async fn create_default_new_token_opportunity(&self, chain_id: u32, token_address: String, alert: MempoolAlert) {
        ...
        let opportunity = MevOpportunity {
            ...
            expires_at: now + (MAX_OPPORTUNITY_AGE_MS / 1000),
            executed: false,
            estimated_profit_usd: 0.0, // Không ước tính lợi nhuận vì không đủ dữ liệu
            estimated_gas_cost_usd: 15.0, // Ước tính chi phí gas
            estimated_net_profit_usd: 0.0,
            token_pairs: Vec::new(), // Chưa biết cặp token
            risk_score: 80.0, // Token mới không có đủ thông tin thường có rủi ro cao
            related_transactions: alert.related_transactions,
            execution_method: MevExecutionMethod::StandardTransaction,
            specific_params: {
                let mut params = HashMap::new();
        ...
        // Thêm vào danh sách cơ hội
        let config = self.config.read().await;
        if config.max_risk_score >= 80.0 { // Chỉ thêm nếu người dùng chấp nhận rủi ro cao
            let mut opportunities = self.opportunities.write().await;
            opportunities.push(opportunity);
        }
    }
    ...
    pub async fn evaluate_opportunities(&self) {
        ...
        opportunities.sort_by(|a, b| {
            b.estimated_net_profit_usd.partial_cmp(&a.estimated_net_profit_usd).unwrap()
        });
    }
