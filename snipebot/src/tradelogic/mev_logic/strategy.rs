...
impl MevStrategy {
    ...
    /// Thực thi một cơ hội MEV
    async fn execute_single_opportunity(&self, opportunity: &MevOpportunity, adapter: &Arc<EvmAdapter>) -> anyhow::Result<f64> {
        // Kiểm tra cấu hình
        let config = self.config.read().await;
        if !config.enabled {
            return Err(anyhow::anyhow!("MEV strategy is disabled"));
        }

        // Kiểm tra rate limit
        let mut executions = self.executions_this_minute.write().await;
        if *executions >= config.max_executions_per_minute {
            return Err(anyhow::anyhow!("Rate limit exceeded: {} executions per minute", config.max_executions_per_minute));
        }

        // Kiểm tra token whitelist
        if let Some(token_pair) = &opportunity.token_pair {
            if !config.token_whitelist.contains(&token_pair.token0) || !config.token_whitelist.contains(&token_pair.token1) {
                return Err(anyhow::anyhow!("Token not in whitelist: {:?}", token_pair));
            }
        }

        // Kiểm tra rủi ro
        if opportunity.risk_score > config.max_risk_score {
            return Err(anyhow::anyhow!("Risk score too high: {} > {}", opportunity.risk_score, config.max_risk_score));
        }

        // Kiểm tra lợi nhuận
        if opportunity.estimated_net_profit_usd < config.min_profit_threshold_usd {
            return Err(anyhow::anyhow!("Profit too low: {} < {}", opportunity.estimated_net_profit_usd, config.min_profit_threshold_usd));
        }

        // Thực thi giao dịch
        let result = match opportunity.execution_method {
            MevExecutionMethod::FlashLoan => {
                self.execute_flash_loan(opportunity, adapter).await
            },
            MevExecutionMethod::Standard => {
                self.execute_standard_trade(opportunity, adapter).await
            },
            MevExecutionMethod::MultiSwap => {
                self.execute_multi_swap(opportunity, adapter).await
            },
            MevExecutionMethod::CustomContract => {
                self.execute_custom_contract(opportunity, adapter).await
            },
            MevExecutionMethod::MevBundle => {
                self.execute_mev_bundle(opportunity, adapter).await
            },
            MevExecutionMethod::PrivateRPC => {
                self.execute_private_rpc(opportunity, adapter).await
            },
            MevExecutionMethod::BuilderAPI => {
                self.execute_builder_api(opportunity, adapter).await
            },
        };

        // Cập nhật số lần thực thi nếu thành công
        if result.is_ok() {
            *executions += 1;
        }
    ...
    async fn execute_flash_loan(&self, opportunity: &MevOpportunity, adapter: &Arc<EvmAdapter>) -> anyhow::Result<f64> {
        ...
        match adapter.execute_flash_loan(
            params.get("loan_token").unwrap(),
            params.get("loan_amount").unwrap().parse()?,
    ...
    async fn execute_custom_contract(&self, opportunity: &MevOpportunity, adapter: &Arc<EvmAdapter>) -> anyhow::Result<f64> {
        ...
        match adapter.execute_custom_contract(
            opportunity.specific_params.get("contract_address").unwrap(),
            &opportunity.related_transactions,
            opportunity.estimated_gas_cost_usd
        ).await {
            Ok(profit) => {
    ...
    async fn execute_mev_bundle(&self, opportunity: &MevOpportunity, adapter: &Arc<EvmAdapter>) -> anyhow::Result<f64> {
        ...
        if opportunity.bundle_data.is_none() {
            return Err(anyhow::anyhow!("Missing bundle data"));
        }

        // Execute MEV bundle
        match adapter.execute_mev_bundle(
            opportunity.bundle_data.as_ref().unwrap(),
            opportunity.estimated_gas_cost_usd
        ).await {
            Ok(profit) => {
                // Update execution stats
    ...
    async fn process_new_token(&self, chain_id: u32, token: NewTokenInfo, alert: MempoolAlert) {
        info!("Phát hiện token mới trên chain {}: {}", chain_id, token.address);
        
        // Lấy adapter cho chain
        let adapter = match self.evm_adapters.get(&chain_id) {
            Some(adapter) => adapter,
            None => {
                error!("Không tìm thấy adapter cho chain {}", chain_id);
                return;
            }
        };

        // Tạo contract info
        let contract_info = self.create_contract_info(chain_id, &token.address, &token, adapter).await;
        
        // Phân tích độ an toàn của token
        let token_status = self.analyze_token_safety(contract_info).await;
        
        // Nếu token an toàn, tạo cơ hội MEV
        if token_status.is_safe() {
            let opportunity = MevOpportunity {
                chain_id,
                token_address: token.address.clone(),
                opportunity_type: MevOpportunityType::NewToken,
                estimated_profit: 0.0, // Sẽ được tính toán sau
    ...
    async fn process_new_liquidity(&self, chain_id: u32, alert: MempoolAlert) {
        info!("Phát hiện liquidity mới trên chain {}: {:?}", chain_id, alert);
        ...
        // Tạo cơ hội MEV cho liquidity mới
        let opportunity = MevOpportunity {
            chain_id,
            token_address: alert.related_token.as_ref().unwrap().address.clone(),
            opportunity_type: MevOpportunityType::LiquidityAdded,
            estimated_profit: 0.0, // Sẽ được tính toán sau
    ...
    async fn process_liquidity_removal(&self, chain_id: u32, alert: MempoolAlert) {
        info!("Phát hiện liquidity bị rút trên chain {}: {:?}", chain_id, alert);
        ...
        // Tạo cơ hội MEV cho liquidity bị rút
        let opportunity = MevOpportunity {
            chain_id,
            token_address: alert.related_token.as_ref().unwrap().address.clone(),
            opportunity_type: MevOpportunityType::LiquidityRemoved,
            estimated_profit: 0.0, // Sẽ được tính toán sau
    ...
    async fn process_front_running_opportunity(&self, chain_id: u32, alert: MempoolAlert) {
        info!("Phát hiện cơ hội front running trên chain {}: {:?}", chain_id, alert);
        
        // Tạo cơ hội MEV cho front running
        let opportunity = MevOpportunity {
            chain_id,
            token_address: alert.related_token.as_ref().unwrap().address.clone(),
            opportunity_type: MevOpportunityType::FrontRunning,
            estimated_profit: 0.0, // Sẽ được tính toán sau
            risk_level: RiskLevel::High,
            status: OpportunityStatus::Pending,
            created_at: SystemTime::now()
    ...
    /// Xử lý cơ hội sandwich attack
    async fn process_sandwich_opportunity(&self, chain_id: u32, alert: MempoolAlert) {
        info!("Phát hiện cơ hội sandwich attack trên chain {}: {:?}", chain_id, alert);
        
        // Tạo cơ hội MEV cho sandwich attack
        let opportunity = MevOpportunity {
            chain_id,
            token_address: alert.related_token.as_ref().unwrap().address.clone(),
            opportunity_type: MevOpportunityType::SandwichAttack,
            estimated_profit: 0.0, // Sẽ được tính toán sau
            risk_level: RiskLevel::High,
            status: OpportunityStatus::Pending,
            created_at: SystemTime::now()
    ...
    /// Xử lý giao dịch của whale
    async fn process_whale_transaction(&self, chain_id: u32, alert: MempoolAlert) {
        info!("Phát hiện giao dịch của whale trên chain {}: {:?}", chain_id, alert);
        
        // Tạo cơ hội MEV cho whale transaction
        let opportunity = MevOpportunity {
            chain_id,
            token_address: alert.related_token.as_ref().unwrap().address.clone(),
            opportunity_type: MevOpportunityType::WhaleTransaction,
            estimated_profit: 0.0, // Sẽ được tính toán sau
            risk_level: RiskLevel::Medium,
            status: OpportunityStatus::Pending,
            created_at: SystemTime::now()
