...
impl MevOpportunity {
    ...
    /// Check if the opportunity is safe to execute
    pub fn is_safe(&self) -> bool {
        // Check if risk score is acceptable (below 70 is considered safe)
        ...
        let now = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap_or_else(|_| {
                tracing::warn!("Failed to get system time, using default value");
        ...
        // Check if the profit is positive and greater than gas cost
        if self.estimated_net_profit_usd <= 0.0 || self.estimated_profit_usd <= self.estimated_gas_cost_usd {
            return false;
        }
        
        // Check if the opportunity has already been executed
        if self.executed {
            return false;
        }
        
        // Check if the opportunity status is not rejected or failed
        if self.mev_status == MevOpportunityStatus::Rejected || 
           self.mev_status == MevOpportunityStatus::Failed {
            return false;
        }
...
/// Trạng thái của bridge
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum BridgeStatus {
    /// Bridge đang hoạt động bình thường
    Active,
    /// Bridge đang bảo trì
    Maintenance,
    /// Bridge đang gặp sự cố
    Error,
    /// Bridge đã bị vô hiệu hóa
    Disabled,
}

/// Fee information for a bridge transaction
#[derive(Debug, Clone)]
pub struct BridgeFee {
    ...
    /// Fee token address (if not native)
...
pub struct MonitorConfig {
    ...
    /// Whether to save failed transactions
    pub save_failed_transactions: bool,
}
...
/// MEV configuration parameters
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MevConfig {
    /// Whether MEV is enabled
    pub enabled: bool,
    ...
    /// Maximum gas price for MEV transactions
    pub max_gas_price: u64,
    
    /// Minimum profit threshold for MEV operations
    pub min_profit_threshold: f64,
    
    /// Maximum slippage allowed for MEV operations
    pub max_slippage: f64,
    
    /// Whether to use private transactions
...
impl Default for MevConfig {
    fn default() -> Self {
        Self {
            ...
            min_profit_threshold: 0.1, // 0.1 ETH
            max_slippage: 0.01, // 1%
            use_private_tx: true,
            use_flashbots: true,
...
pub enum MevOperationType {
    /// Front-running operation
    FrontRun,
    
    /// Back-running operation
    BackRun,
...
/// MEV operation status
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
pub enum MevOperationStatus {
...
/// MEV operation result
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MevOperationResult {
    /// Operation type
    pub operation_type: MevOperationType,
    
    /// Operation status
    pub status: MevOperationStatus,
    
    /// Operation profit in ETH
    pub profit: f64,
...
impl MevOperationResult {
    /// Create a new MEV operation result
    pub fn new(operation_type: MevOperationType) -> Self {
        Self {
            operation_type,
            status: MevOperationStatus::Pending,
            profit: 0.0,
            gas_used: 0,
            gas_price: 0,
            tx_hash: None,
    ...
    pub fn set_completed(&mut self, profit: f64, gas_used: u64, gas_price: u64, tx_hash: H256) {
        self.status = MevOperationStatus::Completed;
        self.profit = profit;
    ...
    pub fn set_failed(&mut self, error: String) {
        self.status = MevOperationStatus::Failed;
    ...
    pub fn set_cancelled(&mut self) {
        self.status = MevOperationStatus::Cancelled;
        self.completed_at = Some(
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .map(|d| d.as_secs())
