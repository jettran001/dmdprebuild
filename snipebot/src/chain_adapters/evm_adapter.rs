...
impl EvmAdapter {
    ...
    pub async fn get_token_price(&self, token_address: &str) -> Result<f64> {
        self.with_retry("get_token_price", 3, 1000, || async {
            // Logic gốc của get_token_price
            self.ensure_initialized().await?;
            // ... existing code ...
            Ok(0.0) // Placeholder
        }).await
        ...
        Fut: std::future::Future<Output = Result<T>>,
    {
                ...
                Err(e) => {
                    ...
                    if retries <= max_retries {
                        warn!(
                            ...
                            operation, retries, max_retries, retry_delay_ms,
        ...
        bail!("RPC call '{}' failed after {} retries", operation, max_retries)
    }
    ...
    /// * Trả về lỗi nếu địa chỉ token không hợp lệ
    /// * Trả về lỗi nếu không thể kết nối với blockchain
    pub async fn get_token_safety(&self, token_address: &str) -> Result<TokenSafety> {
        self.ensure_initialized().await?;
        ...
        // Kiểm tra blacklist hoặc honeypot đơn giản dựa trên địa chỉ token
        let contract_info = self.get_contract_info(token_address).await?;
        
        // Nếu có blacklist hoặc không thể bán token, đánh dấu là nguy hiểm
        if contract_info.has_blacklist.unwrap_or(false) {
            return Ok(TokenSafety::Dangerous);
        }

        // Mặc định trả về an toàn
        Ok(TokenSafety::Safe)
    }
