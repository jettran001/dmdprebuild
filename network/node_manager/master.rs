...
mod tests {
    ...
    async fn test_register_and_list_slaves() {
        ...
        // List slaves
        let slaves = service.list_slaves().await.unwrap();
        assert_eq!(slaves.len(), 1);
        assert_eq!(slaves[0], "test-slave-1");
        
        // Get slave info
        let info = service.get_slave_info("test-slave-1").await.unwrap();
        assert!(info.is_some());
        let retrieved_profile = info.unwrap();
        assert_eq!(retrieved_profile.cpu_cores, 4);
    }
    ...
    async fn test_health_check() {
        ...
        // Đăng ký slave
        service.register_slave("test-slave-2", profile).await.unwrap();
        
        // Health check
        let result = service.health_check("test-slave-2").await.unwrap();
