...
mod tests {
    ...
    async fn test_register_and_get_info() {
        ...
        let info = match info_result {
            Ok(info) => info,
            Err(e) => panic!("Lỗi khi lấy thông tin: {}", e)
        };
        
        assert!(info.is_some(), "Thông tin phải tồn tại sau khi đăng ký");
        
        // Sử dụng match thay vì unwrap
        let retrieved_profile = match info {
            Some(profile) => profile,
            None => panic!("Không tìm thấy profile")
        };
    ...
    async fn test_receive_task() {
        ...
        let output = match result {
            Ok(output) => output,
            Err(e) => panic!("Lỗi khi nhận task: {}", e)
        };
    ...
    async fn test_invalid_operations() {
        ...
        if let Err(error) = result {
            match error {
                SlaveNodeError::NotRegistered(msg) => assert!(msg.contains("Not registered")),
                _ => panic!("Phải là NotRegistered error, nhưng nhận được: {:?}", error)
            }
        }
        ...
        if let Err(error) = result {
            match error {
                SlaveNodeError::InvalidInput(msg) => assert!(msg.contains("empty")),
                _ => panic!("Phải là InvalidInput error, nhưng nhận được: {:?}", error)
            }
        }
        ...
        // Sử dụng match thay vì expect
        match service.register_self("test-master", profile).await {
            Ok(_) => {},
            Err(e) => panic!("Đăng ký với master thất bại một cách không mong muốn: {}", e)
        }
    ...
    async fn test_health_check() {
        ...
        let is_healthy = match result {
            Ok(status) => status,
            Err(e) => panic!("Lỗi khi kiểm tra health: {}", e)
        };
