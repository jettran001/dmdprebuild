...
mod tests {
    ...
    async fn test_register_and_discover() {
        ...
        assert_eq!(nodes[0], "test-node-1");
        
        // Get node info
        let node_info = service.get_node_info("test-node-1").await.unwrap();
        assert!(node_info.is_some());
        let retrieved_profile = node_info.unwrap();
    ...
    async fn test_get_unknown_node() {
        let service = DefaultDiscoveryService::default();
        let result = service.get_node_info("non-existent").await.unwrap();
