...
mod tests {
    ...
    async fn test_schedule_and_get_tasks() {
        ...
        // Đăng ký task
        let task_id = service.schedule_task("test-node-1", "process_data").await.unwrap();
        assert!(!task_id.is_empty(), "Task ID should not be empty");
        assert!(task_id.contains("test-node-1"), "Task ID should contain node ID");
        
        // Lấy danh sách tasks
        let tasks = service.get_scheduled_tasks("test-node-1").await.unwrap();
        assert_eq!(tasks.len(), 1);
        assert_eq!(tasks[0], "process_data");
    }
    ...
    async fn test_cancel_task() {
        ...
        // Đăng ký task
        service.schedule_task("test-node-2", "process_video").await.unwrap();
        ...
        let tasks = service.get_scheduled_tasks("test-node-2").await.unwrap();
    ...
    async fn test_multiple_tasks_per_node() {
        ...
        // Đăng ký nhiều task
        service.schedule_task("multi-node", "task1").await.unwrap();
        service.schedule_task("multi-node", "task2").await.unwrap();
        service.schedule_task("multi-node", "task3").await.unwrap();
        
        // Lấy và kiểm tra
        let tasks = service.get_scheduled_tasks("multi-node").await.unwrap();
        assert_eq!(tasks.len(), 3);
        assert!(tasks.contains(&"task1".to_string()));
        assert!(tasks.contains(&"task2".to_string()));
        assert!(tasks.contains(&"task3".to_string()));
    }
