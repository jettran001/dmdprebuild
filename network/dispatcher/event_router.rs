...
mod tests {
    ...
    async fn test_register_and_emit_event() {
        ...
        router.register_handler(event_type.clone(), handler).await.unwrap();
        ...
        router.emit(event).await.unwrap();
    ...
    async fn test_unregister_handler() {
        ...
        // Hủy đăng ký handler
        router.unregister_handler(&event_type).await.unwrap();
    ...
    async fn test_multiple_handlers() {
        ...
        // Đăng ký handlers
        let event_type = EventType::NodeConnected;
        router.register_handler(event_type.clone(), handler1).await.unwrap();
        router.register_handler(event_type.clone(), handler2).await.unwrap();
