    ...
    fn validate_sdp(sdp: &str) -> Result<(), WebRtcError> {
        ...
        if sdp.len() > MAX_SDP_LENGTH {
            return Err(WebRtcError::ValidationError(
                format!("SDP too large: {} bytes (max: {})", sdp.len(), MAX_SDP_LENGTH)
            ));
        }
        
        // Kiểm tra các dòng cơ bản
        if !sdp.contains("v=0") {
            return Err(WebRtcError::ValidationError("Invalid SDP: missing version".to_string()));
        }
        
        // Kiểm tra an toàn
        if let Err(e) = security::check_xss(sdp, "webrtc_sdp") {
