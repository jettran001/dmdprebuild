...
mod tests {
    ...
    async fn test_libp2p_plugin_init_node() {
        let plugin = Libp2pPlugin::new();
        let (peer_id, keypair) = plugin.init_node().await.unwrap();
    ...
    async fn test_libp2p_plugin_discover_peers() {
        ...
        assert_eq!(peers.len(), 2); // Our mock returns 2 peers
    }
    ...
    async fn test_libp2p_plugin_connect_to_peer_retry_limit() {
        ...
        assert!(res.is_err());
        assert!(res.unwrap_err().contains("Failed to connect to peer"));
    }
