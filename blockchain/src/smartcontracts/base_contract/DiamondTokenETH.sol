...
import "./external/ILayerZeroEndpoint.sol";
import "./external/ILayerZeroReceiver.sol";

/**
 * @title DiamondTokenETH
 * @dev DMD Token triển khai chuẩn ERC-1155 trên Ethereum với khả năng bridging thông qua LayerZero
 */
contract DiamondTokenETH is IERC1155, ILayerZeroReceiver, IERC165 {
    ...
    /**
     * @dev Bridge token to Base (convenience method)
     * @param toAddress Destination address on Base
    ...
    function lzReceive(uint16 _srcChainId, bytes memory _srcAddress, uint64 _nonce, bytes memory _payload) external override {
        ...
        require(_srcAddress.length == trustedRemotes[_srcChainId].length, "Invalid source address length");
    ...
    /**
     * @dev Ước tính phí bridge đến Base
     */
    function estimateBridgeFeeToBase(bytes memory toAddress, uint256 id, uint256 amount) 
        public view returns (uint256 nativeFee, uint256 zroFee) 
    {
        return estimateBridgeFee(BASE_CHAIN_ID, toAddress, id, amount);
    }
    
    /**
     * @dev Ước tính phí bridge đến BSC
     */
    function estimateBridgeeToeBSC(bytes memory toAddress, uint256 id, uint256 amount) 
        public view returns (uint256 nativeFee, uint256 zroFee) 
    {
