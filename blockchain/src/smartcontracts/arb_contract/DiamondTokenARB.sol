...
import "../base_contract/external/IERC1155.sol";
import "../base_contract/external/IERC1155Receiver.sol";
import "../base_contract/external/IERC165.sol";
import "../base_contract/external/ILayerZeroEndpoint.sol";
import "../base_contract/external/ILayerZeroReceiver.sol";

/**
 * @title DiamondTokenARB
 * @dev DMD Token triển khai trên Arbitrum với chuẩn ERC-1155 và khả năng bridging qua LayerZero
 * Được tối ưu hóa cho Arbitrum để tiết kiệm gas và sử dụng lợi thế của rollup
 */
contract DiamondTokenARB is IERC1155, ILayerZeroReceiver, IERC165 {
    ...
    uint16 public constant ETHEREUM_CHAIN_ID = 101; // LayerZero chain ID cho Ethereum
    ...
    // Cache lưu trữ tier người dùng để tối ưu gas - đặc biệt quan trọng trên Arbitrum
    ...
    // Các sự kiện riêng của DiamondToken
    ...
    // Modifiers
    modifier onlyOwner() {
        require(msg.sender == owner, "Chỉ owner mới được gọi");
        _;
    }
    
    modifier whenNotPaused() {
        require(!paused, "Contract đang tạm dừng");
        _;
    }
     ...
     * @param lzEndpoint Address của LayerZero Endpoint trên Arbitrum
     */
    ...
    /**
     * @dev Chuyển token an toàn (tối ưu hóa gas cho Arbitrum)
    ...
    /**
     * @dev Chuyển nhiều token cùng lúc
     * @param from Địa chỉ gửi
     * @param to Địa chỉ nhận
     * @param ids Mảng ID token
     * @param amounts Mảng số lượng tương ứng
     * @param data Data bổ sung
     */
    function safeBatchTransferFrom(
        ...
        for (uint256 i = 0; i < ids.length; ++i) {
            ...
            // Cập nhật tier cache nếu có DMD token
            if (id == DMD_TOKEN_ID) {
                invalidateTierCache(from);
                updateUserTierCache(to);
            }
        ...
        // Gọi onERC1155BatchReceived nếu người nhận là contract
        if (_isContract(to)) {
            try IERC1155Receiver(to).onERC1155BatchReceived(msg.sender, from, ids, amounts, data) returns (bytes4 response) {
    ...
    function invalidateTierCache(address user) public {
        require(
            ...
            "Chỉ owner hoặc người dùng mới có thể xóa cache"
        );
    ...
    /**
     * @dev Bridge token đến một chain khác (tối ưu gas cho Arbitrum)
    ...
    function bridgeToOtherChain(uint16 chainId, bytes memory toAddress, uint256 id, uint256 amount) public payable whenNotPaused {
        require(balanceOf(msg.sender, id) >= amount, "Không đủ số dư token");
        require(chainId != 0, "Chain ID đích không hợp lệ");
        require(trustedRemotes[chainId].length > 0, "Chain đích chưa được tin cậy");
        
        // Burn token trước khi chuyển
        _burn(msg.sender, id, amount);
        
        // Tạo payload để gửi qua LayerZero (đóng gói nhỏ nhất cho tiết kiệm gas trên Arbitrum)
    ...
    /**
     * @dev Bridge token to Ethereum (hàm tiện ích)
     * @param toAddress Địa chỉ đích trên Ethereum
     * @param id Token ID cần bridge
     * @param amount Số lượng token
     */
    function bridgeToEthereum(bytes memory toAddress, uint256 id, uint256 amount) public payable {
        bridgeToOtherChain(ETHEREUM_CHAIN_ID, toAddress, id, amount);
    }
    
    /**
     * @dev Bridge token to BSC (hàm tiện ích)
     * @param toAddress Địa chỉ đích trên BSC
     * @param id Token ID cần bridge
     * @param amount Số lượng token
     */
    ...
    /**
     * @dev Bridge token to NEAR (hàm tiện ích)
     * @param toAddress Địa chỉ đích trên NEAR
     * @param id Token ID cần bridge
     * @param amount Số lượng token
     */
    ...
    /**
     * @dev Bridge token to Base (hàm tiện ích)
     * @param toAddress Địa chỉ đích trên Base
     * @param id Token ID cần bridge
     * @param amount Số lượng token
     */
    function bridgeToBase(bytes memory toAddress, uint256 id, uint256 amount) public payable {
        bridgeToOtherChain(BASE_CHAIN_ID, toAddress, id, amount);
    }
    ...
    function lzReceive(uint16 _srcChainId, bytes memory _srcAddress, uint64 _nonce, bytes memory _payload) external override {
        ...
        require(msg.sender == address(endpoint), "Chỉ LayerZero endpoint mới được gọi hàm này");
        
        // Kiểm tra trusted remote
        require(trustedRemotes[_srcChainId].length > 0, "Chain nguồn chưa được tin cậy");
        
        // Kiểm tra trusted address
        require(_srcAddress.length == trustedRemotes[_srcChainId].length, "Độ dài địa chỉ nguồn không hợp lệ");
        ...
        for (uint256 i = 0; i < sourceAddressLength; i++) {
            if (_srcAddress[i] != trustedRemotes[_srcChainId][i]) {
                revert("Địa chỉ nguồn không đáng tin cậy");
            }
        }
        ...
        require(toAddress != address(0), "Địa chỉ đích không thể là địa chỉ 0");
     ...
     * @param id ID token
     * @param amount Số lượng token
     * @return nativeFee Phí native token (ETH trên Arbitrum)
    ...
    /**
     * @dev Tối ưu hóa gas trên Arbitrum: ước tính phí bridge đến Ethereum
     */
    function estimateBridgeFeeToEthereum(bytes memory toAddress, uint256 id, uint256 amount) 
        public view returns (uint256 nativeFee, uint256 zroFee) 
    {
        return estimateBridgeFee(ETHEREUM_CHAIN_ID, toAddress, id, amount);
    }
    
    /**
     * @dev Kiểm tra nếu contract hỗ trợ interface
     * @param interfaceId ID của interface
     * @return Có hỗ trợ hay không
     */
    ...
    function transferOwnership(address newOwner) public onlyOwner {
        require(newOwner != address(0), "Địa chỉ owner mới không thể là 0");
        owner = newOwner;
    }
    ...
    /**
     * @dev Tối ưu hóa Arbitrum: Mint batch NFT với gas thấp
     * @param to Địa chỉ nhận NFT
     * @param ids Mảng các ID của NFT
     * @param amounts Mảng số lượng tương ứng
     * @param uris Mảng URI metadata
     */
    function mintBatchNFT(
        ...
        for (uint256 i = 0; i < ids.length; ++i) {
            require(ids[i] != DMD_TOKEN_ID, "Không thể mint DMD token qua hàm này");
            _balances[to][ids[i]] += amounts[i];
            _tokenURIs[ids[i]] = uris[i];
            
            emit NFTMinted(to, ids[i], uris[i]);
        }
    ...
    /**
     * @dev Chuyển đổi uint256 thành string - tối ưu gas cho Arbitrum
     */
    ...
    /**
     * @dev Rút ETH từ contract (chỉ owner)
     * @param amount Số lượng ETH cần rút
     */
    function withdrawETH(uint256 amount) public onlyOwner {
        require(address(this).balance >= amount, "Không đủ ETH trong contract");
