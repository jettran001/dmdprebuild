...
pub async fn transfer_token(&self, private_key: &str, to_account: &str, amount: u64) -> Result<String> {
    ...
    for attempt in 0..MAX_RETRY {
        ...
        match self.execute_transfer_transaction(private_key, to_account, amount as f64).await {
            Ok(tx_hash) => {
...
/// Thực thi giao dịch chuyển token trên Solana
async fn execute_transfer_transaction(
    ...
    // Xác thực đầu vào
    if from_address.is_empty() || !from_address.starts_with("So") {
        return Err(anyhow::anyhow!("Địa chỉ nguồn không hợp lệ: {}", from_address));
    }
    ...
    #[cfg(feature = "solana_sdk")]
    {
        ...
        // Parse private key và tạo keypair
        let keypair_bytes = base58::decode(private_key)?;
        if keypair_bytes.len() != 64 {
            return Err(anyhow::anyhow!("Khóa sai định dạng: độ dài {} khác 64", keypair_bytes.len()));
        }
        ...
        let instruction = if let Some(to_acc) = to_token_account {
            ...
            tx.sign(&[&keypair, &new_account], blockhash);
    ...
    {
        // Triển khai sử dụng Solana Web3 API khi không bật feature "solana_sdk"
        let rpc_config = self.get_rpc_config()?;
        let client = reqwest::Client::new();
        
        // Nạp khóa và tạo chữ ký
        let keypair = match base58::decode(private_key) {
            Ok(decoded) => {
                if decoded.len() != 64 {
                    return Err(anyhow::anyhow!("Khóa sai định dạng: độ dài {} khác 64", decoded.len()));
                }
                decoded
            }
            Err(e) => {
                return Err(anyhow::anyhow!("Không thể giải mã khóa: {}", e));
            }
        };
        
        // Lấy thông tin recent blockhash để tạo giao dịch
        let blockhash_response = client.post(&rpc_config.url)
        ...
        let blockhash_json: serde_json::Value = blockhash_response.json().await?;
        
        if let Some(error) = blockhash_json.get("error") {
            error!("Lỗi khi lấy blockhash: {:?}", error);
            return Err(anyhow::anyhow!("Không thể lấy blockhash: {:?}", error));
        }
        
        let blockhash = blockhash_json["result"]["value"]["blockhash"]
            .as_str()
            .ok_or_else(|| anyhow::anyhow!("Không thể đọc blockhash từ phản hồi"))?;
            
        // Lấy token account từ địa chỉ
        // Truy vấn RPC để lấy token account thực tế thay vì giả định
        let get_token_accounts_request = serde_json::json!({
            "jsonrpc": "2.0",
            "id": 1,
        ...
        let token_accounts_response = client.post(&rpc_config.url)
            .json(&get_token_accounts_request)
            .send()
            .await?;
            
        let token_accounts_json: serde_json::Value = token_accounts_response.json().await?;
        
        if let Some(error) = token_accounts_json.get("error") {
            error!("Lỗi khi lấy token accounts: {:?}", error);
            return Err(anyhow::anyhow!("Không thể lấy token accounts: {:?}", error));
        }
        
        // Tìm token account cho token DMD
        let token_accounts = token_accounts_json["result"]["value"].as_array()
            .ok_or_else(|| anyhow::anyhow!("Không thể đọc danh sách token accounts"))?;
            
        let from_token_account = token_accounts.iter()
            .find(|acc| {
                acc["account"]["data"]["parsed"]["info"]["mint"]
                    .as_str()
                    .map_or(false, |mint| mint == self.token_address)
            })
            .map(|acc| acc["pubkey"].as_str().unwrap_or_default())
            .ok_or_else(|| anyhow::anyhow!("Không tìm thấy token account cho DMD token"))?;
            
        // Kiểm tra/tạo token account cho người nhận
        let to_token_accounts_request = serde_json::json!({
            "jsonrpc": "2.0",
            "id": 1,
            "method": "getTokenAccountsByOwner",
            "params": [
        ...
        let to_token_accounts_response = client.post(&rpc_config.url)
            .json(&to_token_accounts_request)
            .send()
            .await?;
            
        let to_token_accounts_json: serde_json::Value = to_token_accounts_response.json().await?;
        
        let to_token_accounts = to_token_accounts_json["result"]["value"].as_array()
            .ok_or_else(|| anyhow::anyhow!("Không thể đọc danh sách token accounts của người nhận"))?;
        ...
        let lamports = (amount * 1_000_000_000.0) as u64; // Chuyển đổi sang lamports/SPL token unit
        
        let transfer_instruction = if let Some(to_acc) = to_token_account {
            // Chuyển token đến account hiện có
            serde_json::json!([{
                "programId": "TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA",
                "accounts": [
                    { "pubkey": from_token_account, "isSigner": false, "isWritable": true },
                ...
                "data": ["3", lamports.to_string(), "0", "0"] // [Transfer instruction index, amount, decimals, options]
            }])
        } else {
            // Cần tạo account mới cho người nhận trước khi chuyển token
            // Trong triển khai thực tế cần nhiều bước: tạo account, khởi tạo, rồi mới chuyển
            // Trường hợp này phức tạp và cần nhiều dữ liệu hơn nên ta sẽ báo lỗi
            return Err(anyhow::anyhow!("Người nhận chưa có token account cho DMD. Hãy tạo token account trước khi chuyển token."));
        };
        
        // Tạo giao dịch để gửi
        ...
        // Gửi giao dịch
        let tx_response = client.post(&rpc_config.url)
            .json(&transaction)
            .send()
            .await?;
        
        let tx_json: serde_json::Value = tx_response.json().await?;
        
        if let Some(error) = tx_json.get("error") {
            error!("Lỗi khi gửi giao dịch: {:?}", error);
            return Err(anyhow::anyhow!("Giao dịch thất bại: {:?}", error));
        }
        
        let tx_hash = tx_json["result"]
            .as_str()
            .ok_or_else(|| anyhow::anyhow!("Không thể đọc hash giao dịch từ phản hồi"))?
            .to_string();
