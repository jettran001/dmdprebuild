 ...
 */
contract ERC1155Wrapper is Ownable, Pausable, ERC1155Holder, ReentrancyGuard {
     ...
     */
    function unwrap(address to, uint256 amount) external whenNotPaused nonReentrant {
        require(msg.sender == address(wrappedDmd), "Only wrapped DMD contract can call");
        require(to != address(0), "Cannot unwrap to zero address");
        
        // Kiểm tra số lượng token có đủ không
        uint256 contractBalance = dmdToken.balanceOf(address(this), DMD_TOKEN_ID);
        require(contractBalance >= amount, "Insufficient ERC1155 balance in contract");
    ...
    /**
     * @dev Hàm emergencyUnwrap cho user khi bridge bị tắt
     * @param to Địa chỉ nhận token ERC1155
     * @param amount Số lượng token cần unwrap trong trường hợp khẩn cấp
     */
    function emergencyUnwrap(address to, uint256 amount) external onlyOwner nonReentrant {
        require(to != address(0), "Cannot unwrap to zero address");
        ...
        // Chuyển ERC1155 từ contract này đến địa chỉ đích
        dmdToken.safeTransferFrom(address(this), to, DMD_TOKEN_ID, amount, "");
