        ...
        for subscription in subscriptions {
                ...
                && subscription.status == SubscriptionStatus::Active 
            {
                ...
                info!(
                    ...
                    SubscriptionUtils::days_remaining(&subscription).unwrap_or(0)
                );
            }
        }
...
mod tests {
    ...
    async fn setup_test_db() -> Arc<Pool<Sqlite>> {
        ...
        .execute(&pool)
        .await
        .expect("Failed to create subscription_events table");
    ...
    async fn test_save_and_get_events() {
        ...
        // Lưu sự kiện
        event_manager.save_event(&event).await.expect("Failed to save event");
        
        // Lấy sự kiện
        let events = event_manager.get_user_events(user_id, None, None)
            .await
            .expect("Failed to get user events");
    ...
    #[tokio::test]
    async fn test_check_expiring_subscriptions() {
        let db_pool = setup_test_db().await;
        let event_manager = EventManager::new(db_pool);
        ...
        event_manager.check_expiring_subscriptions(vec![subscription], 7)
            .await
            .expect("Failed to check expiring subscriptions");
