    ...
    K: Eq + Hash + Clone,
{
    ...
    pub fn insert_with_ttl(&mut self, key: K, value: V, ttl: Duration) {
        ...
        if self.lookup.contains_key(&key) {
            ...
            let index = *self.lookup.get(&key).unwrap();
            ...
            if self.entries.len() >= self.capacity {
                ...
                debug!("Cache đầy, xóa entry ít sử dụng nhất: {:?}", &old_entry.key);
    ...
    C: Cache<K, V> + Send + Sync + 'static,
{
    ...
    pub fn with_config(cache: C, config: CacheConfig) -> Self {
        let metrics = if config.collect_metrics {
            Some(Arc::new(Mutex::new(CacheMetrics::new())))
        } else {
            None
        };
        ...
        Self {
            cache: Arc::new(RwLock::new(cache)),
            ongoing_loads: Arc::new(Mutex::new(HashMap::new())),
