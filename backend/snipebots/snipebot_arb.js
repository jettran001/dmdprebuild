...
class SnipeBot {
    constructor(privateKey, privateRpc) {
        this.web3 = new Web3(privateRpc || 'https://arb1.arbitrum.io/rpc');
        this.account = this.web3.eth.accounts.privateKeyToAccount(privateKey);
        this.web3.eth.accounts.wallet.add(this.account);
        this.mev = new MevMode(this);
        this.nodeId = 'arb-node'; // Giả định node ID cho DePIN
    ...
    async buyToken(tokenAddress, amount, poolId) {
        try {
            ...
            const routerAddress = '0x68b3465833fb72A70ecDF485E0e4C7bD8665Fc45'; // Uniswap V3 Router trên Arbitrum
            ...
            const wethAddress = '0x82aF49447D8a07e3bd95BD0d56f35241523fBab'; // WETH trên Arbitrum
            ...
            console.error('[ARB Buy Error]', error.message);
            throw error;
        }
    }
    ...
    async sellToken(tokenAddress, percentage) {
        try {
            ...
            console.error('[ARB Sell Error]', error.message);
            throw error;
        }
    }
    ...
    async monitorMempoolMEV() {
        ...
        for (const trade of trades) {
            ...
            try {
                ...
                console.error('[ARB MEV Error]', error.message);
            }
        }
    }
    ...
    async monitorMempoolSmart() {
        ...
        for (const trade of trades) {
            ...
            try {
                ...
                console.error('[ARB Smart Trade Error]', error.message);
            }
        }
    }
    ...
    async getTokenPrice(tokenAddress) {
        try {
            ...
            console.error('[ARB getTokenPrice Error]', error.message);
            return 0;
        }
    }
    ...
    async fetchMempoolTrades() {
        return new Promise((resolve, reject) => {
            ...
            const query = gql`
                subscription {
                    EVM(mempool: true, network: arbitrum) {
                        buyside: DEXTrades {
                            Transaction {
                                Hash
                                From
            ...
            const subscription = client.request({ query }).subscribe({
                ...
                error(err) {
                    console.error('[ARB fetchMempoolTrades Error]', err.message);
                    reject(err);
                }
            });
    ...
    notifyUser(message, data) {
        if (io) {
            io.emit('tradeHistory', { type: message, ...data, chain: 'arb', timestamp: Date.now() });
        }
