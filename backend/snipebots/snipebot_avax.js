...
const { TraderJoe } = require('@trader-joe/sdk'); // SDK của Trader Joe (nếu có), nếu không thì dùng Uniswap SDK

const depinManager = new DepinManager();

class SnipeBot {
    constructor(privateKey, privateRpc) {
        this.web3 = new Web3(privateRpc || 'https://api.avax.network/ext/bc/C/rpc');
        this.account = this.web3.eth.accounts.privateKeyToAccount(privateKey);
        this.web3.eth.accounts.wallet.add(this.account);
        this.mev = new MevMode(this);
        this.nodeId = 'avax-node'; // Giả định node ID cho DePIN
        ...
        // Trader Joe (DEX) setup
        this.routerAddress = '0x60aE616a2155b7e0e43202dB8D0A25aD2d4B3cB'; // Trader Joe Router trên AVAX
        this.wavaxAddress = '0xB31f66AA3C1e785363F0875A1B74E27b85FD66c'; // WAVAX trên AVAX
    }
    ...
    async buyToken(tokenAddress, amount, poolId) {
        try {
            ...
            const path = [this.wavaxAddress, tokenAddress]; // WAVAX -> Token
            ...
            const tx = routerContract.methods.swapExactAVAXForTokens(
                amountOutMin,
                path,
            ...
            console.error('[AVAX Buy Error]', error.message);
            throw error;
        }
    }
    ...
    async sellToken(tokenAddress, percentage) {
        try {
            ...
            const path = [tokenAddress, this.wavaxAddress]; // Token -> WAVAX
            const deadline = Math.floor(Date.now() / 1000) + 60 * 20;

            await tokenContract.methods.approve(this.routerAddress, sellAmount).send({ from: this.account.address });
            const tx = routerContract.methods.swapExactTokensForAVAX(
                sellAmount,
                amountOutMin,
                path,
                this.account.address,
            ...
            console.error('[AVAX Sell Error]', error.message);
            throw error;
        }
    }
    ...
    async monitorMempoolMEV() {
        ...
        for (const trade of trades) {
            ...
            try {
                ...
                console.error('[AVAX MEV Error]', error.message);
            }
        }
    }
    ...
    async monitorMempoolSmart() {
        ...
        for (const trade of trades) {
            ...
            try {
                ...
                console.error('[AVAX Smart Trade Error]', error.message);
            }
        }
    }
    ...
    async getTokenPrice(tokenAddress) {
        try {
            ...
            console.error('[AVAX getTokenPrice Error]', error.message);
            return 0;
        }
    }
    ...
    async fetchMempoolTrades() {
        return new Promise((resolve, reject) => {
            ...
            const query = gql`
                subscription {
                    EVM(mempool: true, network: avalanche) {
                        buyside: DEXTrades {
                            Transaction {
                                Hash
                                From
            ...
            const subscription = client.request({ query }).subscribe({
                ...
                error(err) {
                    console.error('[AVAX fetchMempoolTrades Error]', err.message);
                    reject(err);
                }
            });
    ...
    notifyUser(message, data) {
        if (io) {
            io.emit('tradeHistory', { type: message, ...data, chain: 'avax', timestamp: Date.now() });
        }
