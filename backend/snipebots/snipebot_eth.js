...
class SnipeBot {
    constructor(privateKey, privateRpc) {
        this.web3 = new Web3(privateRpc || 'https://mainnet.infura.io/v3/YOUR_INFURA_PROJECT_ID');
        this.account = this.web3.eth.accounts.privateKeyToAccount(privateKey);
        this.web3.eth.accounts.wallet.add(this.account);
        this.mev = new MevMode(this);
        this.nodeId = 'eth-node'; // Giả định node ID cho DePIN
        ...
        // Uniswap V3 setup for Ethereum
        this.routerAddress = '0xE592427A0AEce92De3Edee1F18E0157C05861564'; // Uniswap V3 Router trên Ethereum
        this.wethAddress = '0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2'; // WETH trên Ethereum
    }
    ...
    async buyToken(tokenAddress, amount, poolId) {
        try {
            ...
            console.error('[ETH Buy Error]', error.message);
            throw error;
        }
    }
    ...
    async sellToken(tokenAddress, percentage) {
        try {
            ...
            console.error('[ETH Sell Error]', error.message);
            throw error;
        }
    }
    ...
    async monitorMempoolMEV() {
        ...
        for (const trade of trades) {
            ...
            try {
                ...
                console.error('[ETH MEV Error]', error.message);
            }
        }
    }
    ...
    async monitorMempoolSmart() {
        ...
        for (const trade of trades) {
            ...
            try {
                ...
                console.error('[ETH Smart Trade Error]', error.message);
            }
        }
    }
    ...
    async getTokenPrice(tokenAddress) {
        try {
            ...
            console.error('[ETH getTokenPrice Error]', error.message);
            return 0;
        }
    }
    ...
    async fetchMempoolTrades() {
        return new Promise((resolve, reject) => {
            ...
            const query = gql`
                subscription {
                    EVM(mempool: true, network: ethereum) {
                        buyside: DEXTrades {
                            Transaction {
                                Hash
                                From
            ...
            const subscription = client.request({ query }).subscribe({
                ...
                error(err) {
                    console.error('[ETH fetchMempoolTrades Error]', err.message);
                    reject(err);
                }
            });
    ...
    notifyUser(message, data) {
        if (io) {
            io.emit('tradeHistory', { type: message, ...data, chain: 'eth', timestamp: Date.now() });
        }
